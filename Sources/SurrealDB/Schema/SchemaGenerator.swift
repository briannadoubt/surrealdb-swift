...
public struct SchemaGenerator: Sendable {
    ...
    /// Extracts field metadata from schema descriptors.
    ///
    /// This uses metadata generated by `@Surreal` / `@SurrealEdge`, or custom
    /// `HasSchemaDescriptor` conformances.
    private static func extractFields<T>(from type: T.Type) throws(SurrealError) -> [FieldInfo] {
        guard let schemaType = type as? any HasSchemaDescriptor.Type else {
            throw SurrealError.encodingError(
                """
                Schema metadata unavailable for \(String(describing: type)).
                Add @Surreal/@SurrealEdge or conform to HasSchemaDescriptor.
                """
            )
        }

        return schemaType._schemaDescriptor.fields
            .filter { !$0.isRelation && !$0.isComputed }
            .map { field in
                FieldInfo(
                    name: field.name,
                    type: field.type.toSurrealQL(),
                    isOptional: field.isOptional
                )
            }
    }
